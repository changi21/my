...
DEFAULT_SCHEDULES = {
    ...
    "화요일": [
        ...
        {"time": "15:00 ~ 17:30", "name": "🏠 할아버지 댁 여유시간", "detail": "💡 독서, 학교/학원 숙제 사전 해결", "badge": "여유시간"},
        {"time": "17:30 ~ 18:30", "name": "🥋 합기도 학원", "detail": "할아버지 댁에서 도보 3분", "badge": "운동"},
        {"time": "18:30 ~ 19:15", "name": "🚘 하원 픽업 & 집 이동", "detail": "18:40 픽업 ➡️ 19:15 집 도착", "badge": "픽업"},
    ],
    "수요일": [
...
DEFAULT_EVENING = {
    "Plan A": [
        ...
        ("20:10 ~ 21:20", "⚡ 자기주도 몰입 학습 (70분)", "수학(30m) ➡️ 영어(15m) ➡️ 국어/어휘(15m) ➡️ 내일가방(10m)"),
        ("21:20 ~ 21:50", "🚿 샤워 & 취침 준비", "21:20 샤워 들어가기 및 소등 준비"),
        ("21:50 ~ 22:10", "🛌 잠자리 취침 완료", "22:00 ~ 22:10 사이 취침"),
    ],
    "Plan B": [
        ("19:15 ~ 19:40", "자기 관리 시간", "부모님 저녁 준비 동안 손 씻기 및 할 일 체크"),
        ("19:40 ~ 20:10", "📖 선(先) 집중 학습 (30분)", "영어 단어 + 독해 1장 먼저 끝내기"),
        ("20:10 ~ 20:50", "저녁 식사", "식사 및 식탁 정돈"),
        ("20:50 ~ 21:20", "📐 메인 학습: 수학 & 마무리", "수학 숙제 마무리 & 책가방 챙기기"),
...
STICKER_ICONS = ["🏆", "⭐", "🥇", "🎯", "🚀", "👑", "🔥", "💎", "🎨", "⚾", "🥋", "📖", "🧠", "⚡", "🌟", "🦁", "🐯", "🦄", "🦅", "🥊", "🎮", "🧩", "💡", "🎓", "🍀", "🌈", "🎖️", "🎗️", "🏅", "✨"]
STICKER_MSG = [
    "오늘도 목표 완수! 정말 대단해!", "꾸준함이 너의 최고의 무기야!", "끝까지 해내는 네가 진짜 영웅!",
    "매일매일 성장하는 모습이 멋져!", "오늘의 저녁 스퍼트 최고였어!", "어려운 퀴즈도 척척 풀다니 최고!",
...
with tab1:
    ...
    with top_c1:
        with st.container(border=True):
            st.markdown('<div style="font-size:11px; font-weight:700; color:#64748b;">🚩 다가오는 주요 일정 D-Day</div>', unsafe_allow_html=True)
    ...
    with st.container(border=True):
        ...
        if not is_weekend:
            ...
            sch_detail_txt = " ➡️ ".join([f"{item['time'].split('~')[0].strip()} {item['name']}" for item in sch_items]) if sch_items else "등록된 학원 일정이 없습니다."

            with t_cols[0]:
                st.markdown('<div class="tl-item-box"><div><div style="font-size:11px; font-weight:700; color:#2563eb;">🌅 06:30 ~ 08:30 [아침]</div><div style="font-size:12px; font-weight:700; color:#0f172a; margin:3px 0;">기상 & 한국사 강의 시청</div></div><div style="font-size:10px; color:#64748b;">할아버지 댁 이동(07:20) 후 아침 식사 ➡️ 08:22 등교</div></div>', unsafe_allow_html=True)
            with t_cols[1]:
                st.markdown(f'<div class="tl-item-box"><div><div style="font-size:11px; font-weight:700; color:#059669;">🏫 08:30 ~ 18:30 [{selected_day} 방과후]</div><div style="font-size:12px; font-weight:700; color:#0f172a; margin:3px 0;">학교 수업 & 학원 동선</div></div><div style="font-size:10px; color:#64748b; line-height:1.3;">{sch_detail_txt}</div></div>', unsafe_allow_html=True)
            with t_cols[2]:
                st.markdown('<div class="tl-item-box"><div><div style="font-size:11px; font-weight:700; color:#d97706;">🚘 18:30 ~ 20:10 [귀가&식사]</div><div style="font-size:12px; font-weight:700; color:#0f172a; margin:3px 0;">부모님 픽업 & 저녁 식사</div></div><div style="font-size:10px; color:#64748b;">18:40 픽업 ➡️ 19:15 집 도착 ➡️ 가족 저녁 식사</div></div>', unsafe_allow_html=True)
            with t_cols[3]:
                st.markdown('<div class="tl-item-box"><div><div style="font-size:11px; font-weight:700; color:#4f46e5;">🌙 20:10 ~ 22:10 [저녁&취침]</div><div style="font-size:12px; font-weight:700; color:#0f172a; margin:3px 0;">학습 & 22시 전 취침</div></div><div style="font-size:10px; color:#64748b;">20:10 학습 ➡️ 21:20 샤워 ➡️ 22:00~22:10 취침</div></div>', unsafe_allow_html=True)
        else:
            with t_cols[0]:
                st.markdown('<div class="tl-item-box"><div><div style="font-size:11px; font-weight:700; color:#2563eb;">📝 07:00 ~ 08:30 [주말 아침]</div><div style="font-size:12px; font-weight:700; color:#0f172a; margin:3px 0;">아침 모닝 학습</div></div><div style="font-size:10px; color:#64748b;">수학+영어+독서 집중 완주 후 즐거운 아침 식사</div></div>', unsafe_allow_html=True)
            with t_cols[1]:
                st.markdown('<div class="tl-item-box"><div><div style="font-size:11px; font-weight:700; color:#059669;">🎮 10:00 ~ 12:00 [주말 오전]</div><div style="font-size:12px; font-weight:700; color:#0f172a; margin:3px 0;">아침 게임 & 자유시간</div></div><div style="font-size:10px; color:#64748b;">학습 완주 보상! 규칙에 맞춘 즐거운 게임 시간</div></div>', unsafe_allow_html=True)
            with t_cols[2]:
                st.markdown('<div class="tl-item-box"><div><div style="font-size:11px; font-weight:700; color:#d97706;">⚾ 13:00 ~ 18:00 [주말 오후]</div><div style="font-size:12px; font-weight:700; color:#0f172a; margin:3px 0;">신체활동 & 외출</div></div><div style="font-size:10px; color:#64748b;">13:00 야구/신체활동 ➡️ 15:30 자유시간 및 외출</div></div>', unsafe_allow_html=True)
            with t_cols[3]:
                st.markdown('<div class="tl-item-box"><div><div style="font-size:11px; font-weight:700; color:#4f46e5;">🎲 18:00 ~ 21:00 [주말 저녁]</div><div style="font-size:12px; font-weight:700; color:#0f172a; margin:3px 0;">가족 식사·소통 & 몰입 독서</div></div><div style="font-size:10px; color:#64748b;">18시 저녁식사 ➡️ 19시 보드게임/대화 ➡️ 20시 독서</div></div>', unsafe_allow_html=True)
...
# ==========================================
with tab2:
    st.markdown('<div class="banner-blue">🎒 <b>방과 후 학원 동선:</b> 합기도는 매일 17:30(도보 3분), 수요일은 피아노와 수학이 같은 건물 옆 호실로 연속 수강됩니다.</div>', unsafe_allow_html=True)
    edit_weekday = st.toggle("✏️ 일정 수정 모드 켜기/끄기", key="tog_weekday")
    day_choice = st.radio("요일 선택:", ["월요일", "화요일", "수요일", "목요일", "금요일"], horizontal=True, label_visibility="collapsed")
    items = st.session_state.schedules[day_choice]
    st.markdown("<div style='margin-top: 10px;'></div>", unsafe_allow_html=True)

    if edit_weekday:
        st.warning(f"✏️ **{day_choice} 항목 수정 및 추가/삭제를 완료한 후 '수정 내용 저장' 버튼을 누르세요.**")
        new_items, to_delete = [], None
        for idx, item in enumerate(items):
            c1, c2, c3, c4 = st.columns([2, 3, 4, 1.2])
            with c1: t_val = st.text_input(f"시간 #{idx+1}", item["time"], key=f"t_{day_choice}_{idx}")
            with c2: n_val = st.text_input(f"활동명 #{idx+1}", item["name"], key=f"n_{day_choice}_{idx}")
            with c3: d_val = st.text_input(f"상세설명 #{idx+1}", item["detail"], key=f"d_{day_choice}_{idx}")
            with c4:
                st.write(""); st.write("")
                if st.button("🗑️ 삭제", key=f"del_sch_{day_choice}_{idx}"): to_delete = idx
            new_items.append({"time": t_val, "name": n_val, "detail": d_val, "badge": item.get("badge", "일정")})
        ...
        with btn_col1:
            if st.button("➕ 학원 일정 추가"):
                new_items.append({"time": "19:00 ~ 20:00", "name": "새 활동", "detail": "상세 내용 입력", "badge": "일정"})
                st.session_state.schedules[day_choice] = new_items
                st.rerun()
        with btn_col2:
        ...
        with st.container(border=True):
            st.markdown(f'<h3 style="margin-top:0; font-weight:800; color:#0f172a; font-size:16px; margin-bottom:12px;">{day_choice} 일정 상세 보기</h3>', unsafe_allow_html=True)
            for item in items:
                st.markdown(
                    f"""
...
# ==========================================
with tab3:
    st.markdown('<div class="banner-blue">🌙 <b>저녁 루틴 모드 선택:</b> 퇴근 및 저녁 준비 상태에 따라 Plan A(평소)와 Plan B(유연)를 선택하세요.</div>', unsafe_allow_html=True)
    edit_evening = st.toggle("✏️ 저녁 루틴 수정 모드 켜기/끄기", key="tog_evening")
    plan_mode = st.radio("루틴 모드 선택:", ["Plan A (평소 루틴)", "Plan B (유연 루틴)"], horizontal=True, label_visibility="collapsed")
    plan_key = "Plan A" if "Plan A" in plan_mode else "Plan B"

    if edit_evening:
        st.warning(f"✏️ **{plan_key} 루틴 내용을 수정 후 저장 버튼을 누르세요.**")
        new_plan, to_del_eve = [], None
        for idx, (t, n, d) in enumerate(st.session_state.evening_plans[plan_key]):
            c1, c2, c3, c4 = st.columns([2, 3, 4, 1.2])
            with c1: t_val = st.text_input(f"시간 #{idx+1}", t, key=f"et_{plan_key}_{idx}")
            with c2: n_val = st.text_input(f"제목 #{idx+1}", n, key=f"en_{plan_key}_{idx}")
            with c3: d_val = st.text_input(f"상세 #{idx+1}", d, key=f"ed_{plan_key}_{idx}")
            with c4:
                st.write(""); st.write("")
                if st.button("🗑️ 삭제", key=f"del_eve_{plan_key}_{idx}"): to_del_eve = idx
        ...
        with st.container(border=True):
            st.markdown(f'<h3 style="margin-top:0; font-weight:800; color:#0f172a; font-size:16px; margin-bottom:12px;">🟢 {plan_key} 상세 단계</h3>', unsafe_allow_html=True)
            for idx, (t, n, d) in enumerate(st.session_state.evening_plans[plan_key]):
                st.markdown(
                    f"""
...
# ==========================================
with tab4:
    st.markdown('<div class="banner-blue">☀️ <b>주말 타임라인:</b> 알찬 아침 모닝 90분 공부, 오후 야외활동, 그리고 가족 소통 시간 중심 구성입니다.</div>', unsafe_allow_html=True)
    edit_weekend = st.toggle("✏️ 주말 일정 수정 모드 켜기/끄기", key="tog_weekend")
    weekend_choice = st.radio("주말 요일 선택:", ["토요일 타임라인", "일요일 타임라인 (예배 포함)"], horizontal=True, label_visibility="collapsed")
    wk_key = "토요일" if "토요일" in weekend_choice else "일요일"

    if edit_weekend:
        st.warning(f"✏️ **{wk_key} 수정을 완료한 후 '수정 내용 저장' 버튼을 누르세요.**")
        new_wk_plan, to_del_wk = [], None
        for idx, (t, n, d) in enumerate(st.session_state.weekend_plans[wk_key]):
            c1, c2, c3, c4 = st.columns([2, 3, 4, 1.2])
            with c1: t_val = st.text_input(f"시간 #{idx+1}", t, key=f"wt_{wk_key}_{idx}")
            with c2: n_val = st.text_input(f"할 일 #{idx+1}", n, key=f"wn_{wk_key}_{idx}")
            with c3: d_val = st.text_input(f"비고 #{idx+1}", d, key=f"wd_{wk_key}_{idx}")
            with c4:
                st.write(""); st.write("")
                if st.button("🗑️ 삭제", key=f"del_wk_{wk_key}_{idx}"): to_del_wk = idx
        ...
        with st.container(border=True):
            st.markdown(f'<h3 style="margin-top:0; font-weight:800; color:#0f172a; font-size:16px; margin-bottom:12px;">🗓️ {wk_key} 상세 일정</h3>', unsafe_allow_html=True)
...
with tab5:
    ...
    if edit_chk:
        st.warning("✏️ **체크리스트 미션 문구를 자유롭게 수정한 뒤 저장 버튼을 누르세요.**")
        col_ew, col_ewk = st.columns(2)
        with col_ew:
            st.write("### 📅 평일 미션 문구 수정")
            new_w_chk = {k: (st.text_input(f"평일 미션 {k}", txt, key=f"etxt_w_{k}"), val) for k, (txt, val) in st.session_state.checklist_weekday.items()}
        with col_ewk:
            st.write("### ☀️ 주말 미션 문구 수정")
            new_wk_chk = {k: (st.text_input(f"주말 미션 {k}", txt, key=f"etxt_wk_{k}"), val) for k, (txt, val) in st.session_state.checklist_weekend.items()}
        ...
        with col_w:
            with st.container(border=True):
                st.markdown('<h3 style="margin-top:0; font-weight:800; color:#0f172a; font-size:15px; margin-bottom:12px;">📅 평일 필수 미션</h3>', unsafe_allow_html=True)
                for key, (text, val) in st.session_state.checklist_weekday.items():
                    ...
                    if checked != val:
                        ...
                        w_cnt = sum(1 for _, v in st.session_state.checklist_weekday.values() if v)
                        today_m = today_str[5:]
        ...
        with col_wk:
            with st.container(border=True):
                st.markdown('<h3 style="margin-top:0; font-weight:800; color:#0f172a; font-size:15px; margin-bottom:12px;">☀️ 주말 필수 미션</h3>', unsafe_allow_html=True)
                for key, (text, val) in st.session_state.checklist_weekend.items():
                    checked = st.checkbox(text, value=val, key=f"check_{key}")
                    if checked != val:
                        st.session_state.checklist_weekend[key] = (text, checked)
                        wk_cnt = sum(1 for _, v in st.session_state.checklist_weekend.values() if v)
                        today_m = today_str[5:]
                        hist = [h for h in st.session_state.history_log if h.get("date") != today_m]
        ...
        with st.container(border=True):
            ...
            if not logs:
                st.markdown('<div style="text-align:center; padding:20px; font-size:12px; color:#94a3b8;">아직 저장된 과거 기록이 없습니다. 위 체크리스트를 클릭해 보세요!</div>', unsafe_allow_html=True)
            else:
                h_cols = st.columns(7)
                for idx in range(7):
                    with h_cols[idx]:
                        if idx < len(logs):
                            lg = logs[idx]
                            c_val, tot_val, dt_val = lg.get("count", 0), lg.get("total", 5), lg.get("date", "-")
                            is_success = c_val >= 4
                            bg_style = "background-color:#eff6ff; border:1px solid #bfdbfe; color:#1e40af;" if is_success else "background-color:#f8fafc; border:1px solid #cbd5e1; color:#475569;"
                            st.markdown(f'<div style="{bg_style} border-radius:12px; padding:10px 4px; text-align:center; margin-bottom:4px;"><div style="font-size:10px; font-weight:700;">{dt_val}</div><div style="font-size:16px; font-weight:900; margin:4px 0;">{c_val} / {tot_val}</div><div style="font-size:10px; font-weight:700;">{"🎉 완수" if is_success else "참 잘했어요"}</div></div>', unsafe_allow_html=True)
                        else:
                            st.markdown('<div style="background-color:#f8fafc; border:1px dashed #cbd5e1; border-radius:12px; padding:10px 4px; text-align:center; color:#94a3b8; margin-bottom:4px;"><div style="font-size:10px;">대기</div><div style="font-size:16px; font-weight:700; margin:4px 0;">-</div><div style="font-size:10px;">-</div></div>', unsafe_allow_html=True)
            st.markdown("<div style='margin-bottom:4px;'></div>", unsafe_allow_html=True)
...
# ==========================================
with tab6:
    st.markdown('<div class="banner-ai-grad"><div style="font-size:11px; font-weight:800; opacity:0.9; margin-bottom:4px; text-transform:uppercase; letter-spacing:0.05em;">GEMINI AI ENGINE POWERED</div><h2 style="margin:0; font-weight:900; font-size:20px;">✨ 초등 5학년 Gemini AI 스마트 학습 코치</h2><p style="margin-top:4px; margin-bottom:0; font-size:12px; opacity:0.95;">AI가 퀴즈 생성, 음성 응원 칭찬, 칭찬 스티커 생성, 그리고 역사/과학 궁금증 답변까지 도웁니다!</p></div>', unsafe_allow_html=True)

    row1_col1, row1_col2 = st.columns(2)
    with row1_col1:
        with st.container(border=True):
            st.markdown('<div><h3 style="margin-top:0; font-weight:800; color:#0f172a; font-size:15px;">🧠 AI 1분 퀴즈 생성기</h3><p style="font-size:11px; color:#64748b; margin-bottom:12px;">아동용 맞춤 퀴즈 또는 실제 한능검 심화 성인 난이도 퀴즈를 무제한 생성합니다!</p></div>', unsafe_allow_html=True)
            subject = st.selectbox("과목 및 출제 유형 선택", ["📜 [AI 무작위] 한국사 퀴즈 (초등 5학년 맞춤)", "🏛️ [실제 유형] 한국사능력검정시험 (한능검 심화/성인용)", "🔬 초등 과학교과 (전 범위 탐구 유형)", "🔤 초등 필수 영단어 & 표현 (영어 발음 듣기 지원)"], key="grid_subject")
    ...
    with row1_col2:
        with st.container(border=True):
            st.markdown('<div><h3 style="margin-top:0; font-weight:800; color:#0f172a; font-size:15px;">🔊 AI 멘토 음성 응원 메시지 (TTS)</h3><p style="font-size:11px; color:#64748b; margin-bottom:12px;">상황과 기분에 맞게 선택해 보세요! 매일 다른 칭찬과 응원 멘트가 전해집니다.</p></div>', unsafe_allow_html=True)
            sit_target = st.selectbox("상황 선택", ["🌅 1. 아침 일찍 일어나 기상 미션을 완수했을 때", "🏫 2. 방과 후 학원 3곳을 모두 무사히 다녀왔을 때", "⚡ 3. 저녁 70분 몰입 학습(수학+영어+국어)을 다 끝냈을 때", "⚾ 4. 주말 아빠와 야구/운동 활동을 완수했을 때", "📖 5. 밤 1시간 몰입 독서를 마쳤을 때", "😊 6. 기분이 아주 좋고 자신감이 넘칠 때", "🌧️ 7. 공부나 문제 풀이가 잘 안되어 속상할 때", "😴 8. 하루 일과가 끝나고 너무 피곤하고 지칠 때", "🔥 9. 시험이나 퀴즈를 앞두고 의욕을 다질 때", "🛌 10. 밤 22시 전 샤워 후 잠자리에 누울 때"], key="grid_sit")

            if st.button("🎙️ AI 응원 음성 들려주기", key="btn_g_voice", use_container_width=True):
                with st.spinner("AI 멘토가 응원 메시지를 작성 중입니다..."):
                    prompt = f"학생 '에릭'의 현재 상황: '{sit_target}'. 이 상황에 맞게 학생 이름 '에릭'을 부르고 50자~100자 사이로 따스하고 다정한 응원 문구를 완성해줘."
    ...
    with row2_col1:
        with st.container(border=True):
            ...
            with c_in:
                new_goal_val = st.text_input("보상 목표", value=st.session_state.reward_goal, key="input_reward_goal", label_visibility="collapsed")
            with c_btn_pin:
    ...
    with row2_col2:
        with st.container(border=True):
            st.markdown('<div><h3 style="margin-top:0; font-weight:800; color:#0f172a; font-size:15px;">🔍 AI 탐구 & 호기심 질의응답 (구글 검색 연동)</h3><p style="font-size:11px; color:#64748b; margin-top:4px; margin-bottom:12px;">역사, 과학, 수학 개념 등 궁금한 점을 최신 정보로 정확하게 답변해 줍니다.</p></div>', unsafe_allow_html=True)
            q_input = st.text_input("질문 입력", "조선 시대 임진왜란 때 이순신 장군 3대 대첩이 뭐야?", key="grid_q_in")

            if st.button("🟢 질문하기", key="btn_g_q", use_container_width=True):
                with st.spinner("눈높이에 맞춰 정리하는 중입니다..."):
                    prompt = f"당신은 초등학생 대상 지식 백과 튜터입니다. 질문: '{q_input}'.\n이해하기 쉽게 1) 핵심 요약 2) 상세 설명 3) 💡 기억할 점 3단계 구조로 정리해 주세요."
                    res_text = call_gemini_api(prompt)
                    st.success("답변 완료!")
