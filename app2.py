...
<style>
    ...
    .metric-val { font-size: 24px; font-weight: 800; color: #1e293b; }
...
with h_col1:
    st.markdown(
        ...
        <div style="display: flex; align-items: center; gap: 10px; margin-bottom: 2px;">
            ...
            <span style="background: #e0e7ff; color: #4338ca; font-size: 11px; font-weight: 700; padding: 2px 8px; border-radius: 20px;">✨ AI Powered</span>
        </div>
...
with tab1:
    st.markdown(
        ...
        <div class="info-banner" style="display:flex; justify-content:space-between; align-items:center;">
            ...
            <span style="background:#ffffff; color:#0284c7; padding:2px 10px; border-radius:20px; font-weight:700; font-size:11px; border:1px solid #bae6fd;">🔥 실시간 연동 중</span>
        </div>
        """,
        unsafe_allow_html=True,
    )
    ...
    with c1:
        with st.container(border=True):
            st.markdown(
                f"<h4 style='margin:0; font-weight:700; color:#1e293b;'>📊 {selected_day} 시간 배분 비율</h4>",
                unsafe_allow_html=True,
            )
            st.caption("하루 루틴의 균형 시각화")
    ...
    with c2:
        with st.container(border=True):
            st.markdown(
                "<h4 style='margin:0; font-weight:700; color:#1e293b;'>🎯 저녁 70분 몰입 학습 과목 구성</h4>",
                unsafe_allow_html=True,
            )
            st.caption("15-30분 단위 숏 스퍼트 구성")
    ...
    with st.container(border=True):
        st.markdown(
            f"<h4 style='margin:0; font-weight:700; color:#1e293b;'>⚡ {selected_day} 실시간 타임라인</h4>",
            unsafe_allow_html=True,
        )
        st.markdown(
                ...
                with cols[idx]:
                    if is_weekend:
                        st.markdown(
                            f"""
                            <div class="tl-card">
                                <div style="font-size:11px; font-weight:700; color:#2563eb;">{item[0]}</div>
                                <div style="font-size:13px; font-weight:700; color:#0f172a; margin:3px 0;">{item[1]}</div>
                        ...
                        st.markdown(
                            f"""
                            <div class="tl-card">
                                <div style="font-size:11px; font-weight:700; color:#2563eb;">{item['time']}</div>
                                <div style="font-size:13px; font-weight:700; color:#0f172a; margin:3px 0;">{item['name']}</div>
...
with tab2:
    ...
    if edit_weekday:
        ...
        with st.container(border=True):
            st.markdown(
                f"<h4 style='margin:0 0 12px 0; font-weight:700; color:#0f172a;'>{day_choice} 일정 상세 보기</h4>",
                unsafe_allow_html=True,
            )
            for item in items:
                st.markdown(
                    f"""
                    <div style="display:flex; justify-content:space-between; align-items:center; padding: 12px 16px; background-color:#f8fafc; border-radius:10px; margin-bottom:8px; border:1px solid #f1f5f9;">
...
with tab3:
    ...
    if edit_evening:
        ...
        with st.container(border=True):
            st.markdown(
                f"<h4 style='margin:0 0 12px 0; font-weight:700; color:#0f172a;'>🟢 {plan_key} 상세 단계</h4>",
                unsafe_allow_html=True,
            )
            for idx, (t, n, d) in enumerate(
                st.session_state.evening_plans[plan_key]
                ...
                st.markdown(
                    f"""
                    <div style="padding: 12px 16px; background-color:#f8fafc; border-radius:10px; margin-bottom:8px; border:1px solid #f1f5f9;">
                        <div style="display:flex; align-items:center; gap:10px;">
                            <span style="font-size:11px; font-weight:700; color:#ef4444; background:#fee2e2; padding:2px 6px; border-radius:4px;">{t}</span>
                            <span style="font-size:14px; font-weight:700; color:#1e293b;">{n}</span>
                        </div>
                        <div style="font-size:12px; color:#64748b; margin-top:4px;">{d}</div>
                    </div>
                    """,
                    unsafe_allow_html=True,
                )
...
with tab4:
    ...
    if edit_weekend:
        ...
        with st.container(border=True):
            st.markdown(
                f"<h4 style='margin:0 0 12px 0; font-weight:700; color:#0f172a;'>🗓️ {wk_key} 상세 일정</h4>",
                unsafe_allow_html=True,
            )
...
with tab5:
    ...
    if edit_chk:
        ...
        with col_w:
            with st.container(border=True):
                st.markdown(
                    "<h4 style='margin:0 0 10px 0; font-weight:700; color:#0f172a;'>📅 평일 필수 미션</h4>",
                    unsafe_allow_html=True,
                )
                for key, (
                    text,
        ...
        with col_wk:
            with st.container(border=True):
                st.markdown(
                    "<h4 style='margin:0 0 10px 0; font-weight:700; color:#0f172a;'>☀️ 주말 필수 미션</h4>",
                    unsafe_allow_html=True,
                )
                for key, (
                    text,
        ...
        with st.container(border=True):
            st.markdown(
                ...
                <div style="text-align:center;">
                    ...
                    <p style="font-size:15px; font-weight:700; color:#2563eb; margin:6px 0 0 0;">
...
with tab6:
    st.markdown(
        """
        <div class="ai-banner">
            <div style="font-size:11px; font-weight:700; opacity:0.9; margin-bottom:2px;">GEMINI AI ENGINE POWERED</div>
    ...
    if "1분 AI 퀴즈" in ai_tool:
        with st.container(border=True):
            st.markdown(
                "<h4 style='margin:0; font-weight:700; color:#0f172a;'>🧠 1분 AI 퀴즈 생성기</h4>",
                unsafe_allow_html=True,
            )
            st.caption(
                "아침 시청 복습이나 저녁 공부 시작 전, 재미있는 1분 퀴즈로 뇌를"
                " 세워보세요!"
            )
            subject = st.radio(
                "퀴즈 과목 선택:",
    ...
    # 2. AI 응원 멘트
        ...
        with st.container(border=True):
            st.markdown(
                "<h4 style='margin:0; font-weight:700; color:#0f172a;'>🔊 AI 멘토 음성 응원 메시지 (TTS)</h4>",
                unsafe_allow_html=True,
            )
            st.caption(
                "상황과 기분에 맞게 선택해 보세요! 매일 다른 칭찬과 응원 멘트가"
                " 전해집니다."
            )
            sit_target = st.radio(
                "💬 현재 상황이나 기분 선택 (10가지):",
    ...
    # 3. 칭찬 스티커
        ...
        with st.container(border=True):
            st.markdown(
                "<h4 style='margin:0; font-weight:700; color:#0f172a;'>🎨 AI 미션 달성 칭찬 스티커 카운터</h4>",
                unsafe_allow_html=True,
            )
            st.caption(
                "오늘 미션을 성공했을 때 칭찬 스티커 카드를 생성하여 내"
                " 스티커북(30개판)에 저장합니다!"
            )
            ...
            if st.session_state.latest_draw_sticker:
                ...
                st.markdown(
                    ...
                    <div style="text-align:center; padding: 14px; background: linear-gradient(135deg, #fbcfe8, #e0e7ff); border-radius:12px; margin-top: 12px;">
                        <div style="font-size: 46px;">{lstk['icon']}</div>
            ...
            st.markdown(
                f"""
                <div style="margin-top:16px; font-size:13px; font-weight:700; color:#475569;">
                    ...
                    if i < total_stickers:
                        with row_cols[c_idx]:
                            if i < len(st.session_state.stickers):
                                ...
                                st.markdown(
                                    f"""
                                    <div style="height:70px; display:flex; flex-direction:column; align-items:center; justify-content:center; border:1px solid #f472b6; border-radius:8px; padding:3px; background-color:#fff5f5; margin-bottom:6px;">
                                ...
                                st.markdown(
                                    f"""
                                    <div style="height:70px; display:flex; align-items:center; justify-content:center; border:1px dashed #cbd5e1; border-radius:8px; color:#cbd5e1; font-size:12px; margin-bottom:6px; background-color:#ffffff;">
                                        {i+1}
                                    </div>
                                    """,
                                    unsafe_allow_html=True,
    ...
    # 4. AI 궁금증 질의응답
        ...
        with st.container(border=True):
            st.markdown(
                "<h4 style='margin:0; font-weight:700; color:#0f172a;'>🔍 AI 탐구 & 호기심 질의응답</h4>",
                unsafe_allow_html=True,
            )
            st.caption(
                "역사, 과학, 수학 등 궁금한 점을 최신 정보로 정확하게 답변해"
                " 줍니다."
            )
            ...
            if st.button("🟢 질문하기", use_container_width=True):
                with st.spinner("눈높이에 맞춰 정리하는 중입니다..."):
                    prompt = (
                        ...
                        " 내용일 경우, 학습 데이터 시점 한계로 인해 실시간 변동될"
                        " 수 있다는 점을 부드럽게 밝히고 가장 신뢰성 있는"
                        " 역사/기본 개념 위주로 설명하세요.\n\n[출력"
                        " 구조]\n이해하기 쉽게\n1) 핵심 요약\n2) 상세 설명\n3) 💡"
