    ...
    with row2_col1:
        with st.container(border=True):
            ...
            if st.session_state.latest_draw_sticker:
                ...
                st.markdown(
                    f"""
                    <div style="text-align:center; padding: 10px; background: linear-gradient(135deg, #fbcfe8, #e0e7ff); border-radius:10px; margin-top: 8px;">
                        <span style="font-size: 24px;">{lstk['icon']}</span>
                ...
                st.markdown(
                    """
                    <div style="height:60px; display:flex; align-items:center; justify-content:center; background:#f8fafc; border:1px dashed #cbd5e1; border-radius:10px; font-size:11px; color:#94a3b8; margin-top:8px;">
                        버튼을 누르면 칭찬 스티커가 생성되어 내 스티커북에 저장됩니다.
                    </div>
                    """,
                    unsafe_allow_html=True,
                )
            ...
            st.markdown(
                f"""
                <div style="margin-top:12px; font-size:12px; font-weight:700; color:#475569; display:flex; justify-content:space-between;">
                    <span>🎯 30개 달성 보상 목표: {st.session_state.reward_goal}</span>
                    <span style="color:#2563eb;">{len(st.session_state.stickers)} / 30개 ({int(len(st.session_state.stickers)/30*100)}%)</span>
                </div>
                """,
                unsafe_allow_html=True,
            )
